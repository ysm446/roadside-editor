...
侵食のUIとノードシステムは、既存プロジェクト **terrain-editor** を土台として利用する。

- GitHub: https://github.com/ysm446/terrain-editor
- ローカルクローン: `C:\GitHub\terrain-editor`
...
- アプリのバージョンは `package.json` の `version` を基準にする（ビルドシステムが確定したら実態に合わせて本項を更新すること）。
...
- フロントエンドや型に関わる変更後は、可能な限りビルド（現状の想定は `npm run build`。C++ 構成になった場合は `cmake --build` に読み替え、本項を更新する）を実行する。
...
- `C:\GitHub\terrain-editor\CLAUDE.md` — 土台プロジェクトのアーキテクチャ解説
- `C:\GitHub\terrain-editor\docs\nodes\README.md` — 既存ノードのドキュメント索引
