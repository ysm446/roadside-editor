...
- UV確認 (チェッカー) 表示を追加(表示パネル > UV確認)。リボンのワールドメッシュにUVテストテクスチャ(8色の市松セル + 「列数字+行英字」ラベル)を貼り、2D ビューにも同じチェッカーを重ねる。セル一辺は UVグリッド間隔に従う。オフで通常シェーディングに戻る
- `Ribbon` に Pack Islands オプションを追加。アトラス幅より長いセンターラインを複数の行バンド(UVアイランド)に折り返してパックする。アイランド外は最低標高より低い平坦領域になり、シームでの水・土砂の流出挙動をそのまま観察できる。3D プレビューはアイランドをワールドで連続した道路に再結合する
- デモプロジェクトを約195mの蛇行パス + 3アイランド構成に更新(UV確認オン)
