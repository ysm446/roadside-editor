...
- 2026-07-23: UV確認 (チェッカーテクスチャ) 表示と UVアイランド分割パッキングを追加。チェッカーは頂点カラー経路 (mesh_preview.hlsl に頂点カラーアルベドのビットを追加) で表示し、2D ビューにも同じチェッカーを重ねる。Pack Islands は長いセンターラインをアトラス行バンドに折り返し (§6-7 のシェル分割)、アイランド外は最低標高-1m の平坦領域。約195m・3アイランドでの侵食貫通とワールド再結合を確認。GPU MSE バックエンドでも baseZ/normalZ が保持されることを確認
...
- Pack Islands のシームは §7.1 の「境界で水と土砂が消える」状態そのもの(ゴーストセル交換は未実装)。アイランド端の侵食挙動の観察・比較用であり、品質が必要な場合は単一アトラス(Pack Islands オフ)を使う
